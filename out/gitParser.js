...
exports.resolveMainBranch = resolveMainBranch;
exports.getGitUserEmail = getGitUserEmail;
exports.getCurrentBranch = getCurrentBranch;
exports.scanBranches = scanBranches;
const child_process_1 = require("child_process");
// ── git helpers ──────────────────────────────────────────────────────────────
function git(args, cwd) {
    const r = (0, child_process_1.spawnSync)('git', args, { cwd, encoding: 'utf8', timeout: 15000 });
...
function resolveMainBranch(cwd, configured) {
    const exists = (name) => !!git(['rev-parse', '--verify', name], cwd);
...
function scanBranches(cwd, mainBranch, ignorePrefixes, hoursPerDay, authorEmail, existingTasks) {
    const MAX_DAYS = 30;
    ...
    for (const line of mergeLog.split('\n').filter(Boolean)) {
        ...
        if (!mergeHash || !mergeDate || !parentsStr)
            continue;
        ...
        if (ignorePrefixes.some(p => branchName.startsWith(p)))
            continue;
        if (taskMap.has(branchName))
        ...
        if (authorEmail) {
            ...
            if (firstAuthor.toLowerCase() !== authorEmail)
                continue;
        }
    ...
    for (const branch of localOut.split('\n').map(b => b.trim()).filter(Boolean)) {
        if (branch === mainBranch)
            continue;
        if (ignorePrefixes.some(p => branch.startsWith(p)))
            continue;
        if (taskMap.has(branch))
            continue; // merge history already got it
        ...
        const times = timesRaw.split('\n').filter(Boolean);
        if (hashes.length === 0 || times.length === 0)
            continue;
        const startDate = new Date(times[0]);
        ...
        const mergeStyle = isMerged ? 'squash' : 'unknown';
        const workingDays = Math.min(workingDaysBetween(startDate, endDate), MAX_DAYS);
        const estimatedHours = Math.round(workingDays * hoursPerDay * 10) / 10;
