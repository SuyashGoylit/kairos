...
function statsGrid(s: Stats): string {
  ...
  return `<div class="grid">${cells.map(([label, val]) =>
    `<div class="card"><div class="card-label">${label}</div><div class="card-value">${val}</div></div>`
  ).join('')}</div>`;
}

function estimateBox(e: EstimateResult): string {
  return `
  <div class="estimate">
...
function tableRows(tasks: TaskRecord[], matchSet: Set<string>): string {
  if (tasks.length === 0) {
